...
import com.sodastream.android.VideosActivity;
...
public class VideosAsyncTask extends AsyncTask<String, String, Boolean> {
	...
	public VideosAsyncTask(Activity _activity) {
	...
	protected Boolean doInBackground(String... params) {
		try 
		{
			...
			httpPost = new HttpPost(URLS.VIDEOS_URL);
			...
			if(content.length() >1)
			{
				...
				VideosModule[] arrTemp =  gson.fromJson(content, VideosModule[].class);

				DATA.arrlstVideosModules = new ArrayList<VideosModule>(Arrays.asList(arrTemp));
	...
	protected void onPostExecute(Boolean result) {
		...
		if(result)
		{
			((VideosActivity)activity).loadVideosGrid();
		}
		else
		{
